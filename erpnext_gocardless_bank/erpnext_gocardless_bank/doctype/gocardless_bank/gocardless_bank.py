...
from erpnext_gocardless_bank.libs.gocardless import (
    get_doc,
    clear_doc_cache,
    enqueue_save_bank,
    enqueue_update_bank,
...
class GocardlessBank(Document):
    ...
    def validate(self):
        if self.is_new():
            ...
            if not self.bank:
                throw(_("Please select a bank to link with Gocardless."))
    ...
    def on_trash(self):
        ...
        try:
            frappe.db.delete("Gocardless Sync Log", {"bank": self.name})
        except Exception:
            pass
            ...
            send_bank_error(
                {
                    "bank": self.bank,
                    "error": _(
                        "The Gocardless link data for \"{0}\" is invalid."
                    ).format(self.bank),
                }
            )
            return 0
            ...
            send_bank_error(
                {
                    "bank": self.bank,
                    "error": _(
                        "The Gocardless bank account data for \"{0}\" is invalid."
                    ).format(self.bank),
                }
            )
            return 0
