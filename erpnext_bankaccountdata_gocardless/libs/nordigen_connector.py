...
from .nordigen_api import NordigenApi
...
class NordigenConnector:
    ...
    def request(self, uri, data=None, auth=None, is_list=False, method=None):
        ...
        if status_code != 200 and status_code != 201:
            log_error(response)
            err = NordigenApi.parse_error(response)
            if "list" not in err:
    ...
    def connect(self):
        if not self.connected and self.secret_id and self.secret_key:
            ...
            if not token or not isinstance(token, dict):
                error(_("Unable to connect to Nordigen"), code="GEgr6QZB23")
    ...
    def refresh(self, refresh_token: str):
        token = self.request(
            NordigenApi.refresh_token,
            {"refresh": refresh_token}
        )
        if not token or not isinstance(token, dict):
            error(_("Unable to refresh Nordigen"), code="RzRRkD3xvD")
    ...
    def get_banks(self, country=None, pay_option=False):
        self.connect()
        return self.request(
            NordigenApi.list_banks(country, pay_option),
            None, True, True
        )
    ...
    def get_bank_link(
        ...
        redirect_url = f"{redirect_url}/app/nordigen-bank"
        if docname:
            redirect_url = f"{redirect_url}/{docname}"
        
        data = self.request(
            NordigenApi.bank_link,
            {
                "institution_id": bank_id,
    ...
    def remove_bank_link(self, auth_id):
        self.connect()
        return self.request(
            NordigenApi.bank_accounts(auth_id),
    ...
    def get_accounts(self, auth_id):
        self.connect()
        data = self.request(NordigenApi.bank_accounts(auth_id), auth=True)
        if "error" in data:
            return data
    ...
    def get_account_data(self, account_id):
        self.connect()
        data = self.request(NordigenApi.account_data(account_id), auth=True)
        if "error" in data:
            return data
        ...
        if "status" in data and isinstance(data["status"], dict):
            for k in data["status"].keys():
                if k in NordigenApi.account_status:
                    status = k.title()
                    break
                elif cstr(k).lower() in NordigenApi.account_status_new:
                    status = k.title()
                    break
    ...
    def get_account_balances(self, account_id):
        self.connect()
        data = self.request(NordigenApi.account_balances(account_id), auth=True)
        if "error" in data:
            return data
    ...
    def get_account_details(self, account_id):
        self.connect()
        data = self.request(NordigenApi.account_details(account_id), auth=True)
        if "error" in data:
            return data
    ...
    def get_account_transactions(self, account_id, date_from, date_to):
        self.connect()
        data = self.request(
            NordigenApi.account_transactions(account_id, date_from, date_to),
            auth=True
        )
    ...
    def prepare_entries(self, data):
        return NordigenApi.prepare_transactions(data)