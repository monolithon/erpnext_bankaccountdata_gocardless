...
from .nordigen_common import (
    error,
    log_error,
    log_info,
    to_json
)
from .nordigen_connector import NordigenConnector


_SETTINGS_ = "Nordigen Settings"
_BANK_ = "Nordigen Bank"
_BANK_ACCOUNT_ = "Nordigen Bank Account"
_SYNC_LOG_ = "Nordigen Sync Log"
_SYNC_LIMIT = 4
_SYNC_CACHE_KEY = "nordigen_auto_sync"
...
def get_client():
    ...
    if doc.access_token and get_datetime(doc.access_expiry) > now_dt:
        return NordigenConnector(doc.access_token)
    
    if doc.refresh_token and get_datetime(doc.refresh_expiry) > now_dt:
        client = NordigenConnector()
        client.refresh(doc.refresh_token)
    else:
        client = NordigenConnector(doc.secret_id, doc.secret_key)
...
def enqueue_save_bank(docname, bank, auth_id):
    frappe.enqueue(
        "erpnext_nordigen.libs.nordigen.save_bank",
        job_name=f"nordigen-save-bank-{bank}",
        is_async=True,
...
def enqueue_update_bank(docname, bank, auth_id):
    frappe.enqueue(
        "erpnext_nordigen.libs.nordigen.update_bank",
        job_name=f"nordigen-update-bank-{bank}",
        is_async=True,
        enqueue_after_commit=False,
        docname=docname,
        bank=bank,
    ...
    if not frappe.db.exists(dt, bank_account_name):
        try:
            (frappe.new_doc(dt)
                .update({
                    ...
                    "bank": bank,
                    "account_type": account["account_type"],
                    "nordigen_bank_account_no": account["account_no"],
                    "company": company,
                    "iban": account["iban"],
            ...
            err = _(
                ...
                "the Nordigen bank \"{1}\" to ERPNext."
        ...
        try:
            ...
            err = _(
                ...
                "the Nordigen bank \"{1}\" in ERPNext."
            ).format(account["account"], bank)
            log_error(exc)
...
def store_bank_accounts(docname, accounts):
    ...
    frappe.publish_realtime(
        event="nordigen_reload_bank_accounts",
...
def is_bank_exist(bank, throw=True):
    if not frappe.db.exists(_BANK_, bank):
        if throw:
            error(_("The Nordigen bank {0} doesn't exist.").format(bank), code="czDbdS5YuV")
        return 0
            ...
            if has_error:
                if has_error == total:
                    error(_(
                        ...
                        "Nordigen bank account \"{0}\" of {1} from {2} to {3}."
                    ).format(
                    ...
                    error(_(
                        ...
                        "Nordigen bank account \"{1}\" of {2} from {3} to {4}."
                    ).format(
...
def update_banks_status():
    if (banks := frappe.get_all(
        ...
        try:
            ...
            log_error(exc)
            error(_("Unable to update Nordigen bank auth status"), False, "xYRxZTKz99")
        ...
        try:
            ...
            log_error(exc)
            error(_("Unable to update Nordigen bank accounts status"), False, "Jma9mRWrBD")
        
        frappe.publish_realtime(
            event="nordigen_updated_bank_accounts",
            after_commit=True
        )
...
def sync_bank_account(
    ...
    frappe.enqueue(
        "erpnext_nordigen.libs.nordigen.sync_bank_account_transactions",
        job_name="nordigen-sync-bank-account-transactions-" + account,
        queue="long",
        is_async=True,
        enqueue_after_commit=False,
        settings=settings,
    ...
    if not frappe.db.exists(dt, {"transaction_id": data["transaction_id"]}):
        try:
            entry_data = _dict({
                ...
                "nordigen_transaction_info": data.get("information", ""),
