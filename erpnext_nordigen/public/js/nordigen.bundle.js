...
frappe.provide('frappe.Nordigen');
frappe.provide('frappe.nordigen');
...
frappe.Nordigen = class Nordigen {
    constructor() {
        this.is_ready = false;
        ...
        this.request(
            ...
            function() {
                this.fatal('Unable to get Nordigen plugin status.');
            }
        );
    }
    on_ready(fn) {
    ...
    fatal(msg, args) {
        this.destroy();
        frappe.throw('[Nordigen]: ' + __(msg, args));
    }
    _console() {
        ...
        if ($class(args[0]) !== 'String') args.unshift('[Nordigen]:');
        else args[0] = '[Nordigen]: ' + __(args[0]);
    ...
    request(method, args, callback, error, _freeze) {
        var me = this;
        let opts = {
            method: 'erpnext_nordigen.libs.nordigen.' + method,
...
frappe.nordigen = function() {
    frappe.nordigen._init = frappe.nordigen._init || new frappe.Nordigen();
    return frappe.nordigen._init;
};